...
/*WooCommerce Customized Filter, states or provinces*/
...
function custom_woocommerce_states( $states ) {
  ...
  $states['PH'] = array(
	...
	'74' => 'Zamboanga del Norte',
	
	'75' => 'Zamboanga del Sur',
	
	'76' => 'Zamboanga Sibugay',
	
	'78' => 'Mountain Province',
	
	'79' => 'Batangas',
	
	'80' => 'Masbate',
	
	'81' => 'Misamis Occidental',
...
//separate reviews from comments in WOOCOMMERCE
...

{
...

	{
		...
		//change the 'comment' to 'review' in product review
        if( isset( $_GET['post_type'] ) ) 
...
//set notification of reviews from comment fix, woocommerce
...
//WOOCOMMERCE add description after title in products page
add_action('woocommerce_after_shop_loop_item_title','woocommerce_template_single_excerpt', 5);

//WOOCOMMERCE add image in category page
...
function woocommerce_category_image() {
    if ( is_product_category() ){
	    ...
	    if ( $image ) {
		    echo '<div class="cat-image"><img src="' . $image . '" alt="" /></div>';
		}
	}
}

//Add custom wrapper in WOOCOMMERCE pages
...
//add bcc for woocommerce email notifications, for testing; this includes email received by customer as well
add_filter( 'woocommerce_email_headers', 'add_bcc_all_emails', 10, 2);
 
function add_bcc_all_emails($headers, $object) {
 
    $headers = array();
    $headers[] = 'Bcc: Aurezza Dunque <au@commsenseinc.com>, Karen Sarte <karen.sarte@philcare.com.ph>';
...
function hyperv_custom_order_number( $order_number, $order ) {
	if ( isset( $order_number ) ) {
		...
		$order_year = date_i18n( 'mdy', strtotime( $order->order_date ) );

		//TEST-MMDDYY-00-3021
		$order_number = $order_year . '-00-' . $order_number;
	}

	return $order_number;
}

//terms and conditions in checkout is checked by default
...
/*
 ********************
 * Contact Form 7 Radio Button Mandatory
 ********************
*/
...
add_action( 'wp_ajax_getallDistrict', 'getallDistrict' );
add_action( 'wp_ajax_nopriv_getallDistrict', 'getallDistrict' );
...
function wptuts_styles_with_the_lot(){
    ...
    //wp_register_style( 'custom-style', plugins_url( 'css/bootstrap.css', __FILE__ ), array(), '20120208', 'all' );
    // or
    // Register the style like this for a theme:
    wp_register_style( 'custom-style', get_template_directory_uri() . '/css/bootstrap.css', array(), '20120208', 'all' );
    wp_register_style( 'custom-styles', get_template_directory_uri() . '/css/style.css', array(), '20120208', 'all' );
...
add_action( 'wp_ajax_membersInfo', 'membersInfo' );
add_action( 'wp_ajax_nopriv_membersInfo', 'membersInfo' );
function membersInfo(){
    ...
    ?>
    <div class="metro">
        <input class="form-control" type="text" id="cert_name" value="<?php echo $info->LastName.", ".$info->FirstName." ".$info->MiddleName ?>" placeholder="Name" disabled>
    </div>
<?php
die();
}

add_action( 'wp_ajax_getProvider', 'getProvider' );
add_action( 'wp_ajax_nopriv_getProvider', 'getProvider' );
function getProvider(){
    
    $city = $_POST['area_name'];
    $distct = $_POST['district'];
    $hospital = $_POST['loc_name'];
    $type = $_POST['dtype'];
    $certno = $_POST['certno'];
    $url = 'https://apps.philcare.com.ph/iPhilCare_Mobile/Providers.svc/FindProviders/?Type='.$type.'&City='.$city.'&District='.$distct.'&Hospital='.$hospital.'&Top=100&CertNo='.$certno;
...
add_action( 'wp_ajax_getMap', 'getMap');
add_action( 'wp_ajax_nopriv_getMap', 'getMap');
...
<div id="map_canvas" style="position: relative; background-color: rgb(229, 227, 223); overflow: hidden; height: 347px; width: 100%;"></div>
<script>
     $(function () {
         var lat = <?php echo $lat;?>,
             lng = <?php echo $long;?>,
             latlng = new google.maps.LatLng(lat, lng),
             image = '<?php bloginfo('template_directory'); ?>/img/gpin.png ?>';
